...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		pid_t waiting_process = current->p_queue;
		if (waiting_process > 0)
		  {
		    proc_array[waiting_process].p_state = P_RUNNABLE;
		    proc_array[waiting_process].p_registers.reg_eax = current->p_exit_status;
		  }
		schedule();
	...
	case INT_SYS_WAIT: {
		...
		if (p <= 0 || p >= NPROCS || p == current->p_pid
		...
		else
		  {
		    ...
		    proc_array[p].p_queue = current->p_pid; // keep track of process that will be put to sleep
		    current->p_state = P_BLOCKED; // put process to sleep
		  }
		schedule();
	}
...
do_fork(process_t *parent)
{
  ...
  pid_t pid_num;
  process_t * cur_proc;
  for (pid_num = 1; pid_num < NPROCS; pid_num++)
    {
      cur_proc = &proc_array[pid_num];
      if (cur_proc->p_state == P_EMPTY)
	{
	  cur_proc->p_registers = parent->p_registers;
	  copy_stack(cur_proc, parent);
	  cur_proc->p_registers.reg_eax = 0;
	  cur_proc->p_state = P_RUNNABLE;
	  return pid_num;
	}
    }
	return -1;
...
copy_stack(process_t *dest, process_t *src)
{
	...
	// Start from where proc1 begins and add offset according to pid
	// multiplied by a process's given stack size.
	src_stack_top = PROC1_STACK_ADDR + (src->p_pid)*PROC_STACK_SIZE;
	// Already given through esp.
	src_stack_bottom = src->p_registers.reg_esp;
	
	// Dest's stack top follows same format as src_stack_top
	dest_stack_top = PROC1_STACK_ADDR + (dest->p_pid)*PROC_STACK_SIZE;
	// Simply subtract the dest_stack_top by the how big the stack size
	// is: top - bottom
	dest_stack_bottom = dest_stack_top - (src_stack_top - src_stack_bottom);

	// following memcpy's function syntax: void *memcpy(void *dest,
	// const void *src, size_t n);
	size_t size = src_stack_top - src_stack_bottom;
	memcpy((void *)dest_stack_top, (const void *) src_stack_top, size);
	// dest's esp register needs dest_stack_bottom, paralleling src_stack_bottom
